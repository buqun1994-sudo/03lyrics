...
2. 触发条件：新增或修改歌词源。动作：外部协议留在 `DirectLyricsRepository`，候选选择留在 `LyricsCandidateSelector`；来源先返回目录候选与来源曲目 ID，再由公共规则核验后读取歌词。验证：错误版本、占位内容、超时与竞争结果测试。边界：UI 不解析第三方 JSON，也不得绕过领域选择器直接请求歌词源。
