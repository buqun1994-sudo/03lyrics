...
2. `PublicMediaBrowserSessionRegistry.refresh` 先读取完整标准 Browser 清单，在冷发现或当前来源缺少播放证据时，把清单所属包也纳入既有补充探测。保留公开组件准入、标准协议连接、最多 `8` 连接、超时和一次重试；不增加播放器包名白名单。服务仅向注册表转交唯一仲裁器的“当前有无播放证据”，选中暂停会话不再关闭发现。
3. `MediaSessionArbiter` 冷选与运行交接共用同一份有时效的播放证据：实际位置推进优先于有效新鲜发布位置，后者优先于旧播放标记和恢复偏好；连续暂停快照的推进也有效。重复同值回调保留 `5000ms` 内的观察事实，时间戳或 `active` 单独变化不刷新证据；播放转暂停、停止和错误清除活动证据。沿用 `1500ms` 冷选、`250ms` 交接、偏好缺失等待和既有低频刷新；双方都推进时保留当前来源，停止后不回接无关暂停历史。
4. 生产改动只涉及注册表、仲裁器和服务的一处发现条件；歌词、缓存、录音代际与时间线沿现有主链。新增 Arbiter → Adapter → Store 联动验证：旧蓝牙缓存已显示时，新来源接管先清空旧正文与缓存摘要，再读取新歌内置歌词；新歌无歌词时保持空结果，不能因旧缓存可用而选回蓝牙。生产 HTML 继续拒绝旧代际和旧修订结果。
5. 已完成改动范围代码复核；8 个直接相关测试类共 `192` 项 JVM 测试通过，覆盖仲裁、Browser、元数据、Adapter、时间线、Store 和设置契约。`assembleDebug` 成功；`scripts/test-lyrics-overlay.mjs --no-screenshots` 的 `80` 项生产 HTML 断言通过，覆盖桌面、顶栏和窄顶栏，未截图。脚本仅新增关闭截图选项，行为断言保持一致。
6. Debug 开发包已准备在 `app/build/outputs/apk/debug/app-debug.apk`：`com.ninepointnine.desktoplyrics.test / 1.0.14-icar03-test / 128`，SHA-256 为 `a43e15d3c9464e6f49339f0386325d5b7896d3c19a430f1d60f1cb94afe98f03`。版本真值检查、实际 APK 身份、单 signer 和 APK v2 签名校验通过，未递增版本。此包用于本地实装调试，不是 staging 分发包。
7. 产品、架构、媒体接入规则和验证矩阵已同步“先决定唯一播放来源，再读取对应歌词”；项目文档、Skill、脚本语法和 Git diff 检查通过，同一次实装前直接复用以上测试与产物摘要。共享 03 APP 检查仍因 Cloud 登记为 `1.0.10 / 124` 及旧 HEAD / clean 快照失败，本轮未改写外部登记，不能宣称正式发布预检通过。
8. 用户通知“可以测试了”后复用上述已验证 APK，摘要一致。按既有授权卸载车机正式包并安装 `com.ninepointnine.desktoplyrics.test / 1.0.14-icar03-test / 128`，通过 ADB 设置通知监听和悬浮窗授权；`scripts/install-and-smoke.mjs` 通过，设置页启动、两个表面租约、窗口避让无障碍、媒体监听和歌词服务运行均正常，未发现致命日志。窗口避让授权保留两个既有组件并追加 debug 组件，车上仅保留 debug 歌词包。
9. 实装现场旧蓝牙仍为 `active=true / PAUSED`，网易云为 `active=false / PLAYING`；服务绑定记录确认 debug 直接连接 `CloudMusicService`。只读 WebView 数据确认当前歌曲 `Richest / Muni Long / Richest`、时长 `219000ms`、`46` 行歌词、可信时间线就绪且歌词可见；另一次 `33193ms` 连续观察中位置从 `182692ms` 推进到 `215746ms`，录音代际 / 查询修订始终为 `4 / 4`，没有来源交接、录音变动或重复歌词快照。同期蓝牙旧歌曲未抢占，末段活动行与该歌曲末句一致。证据保存在被忽略的 `app/build/reports/netease/vehicle-20260910/`，只读采样未修改 WebView 播放状态。
10. 同 UID 的 shell `content` 入口受 Android 外部 Provider 访问权限限制，未取得该入口的完整内部诊断轨迹；本次证据来自公开服务绑定、当前应用日志和 debug WebView 只读对象 / DOM。临时日志级别已恢复，调试端口已移除，未截图、未自动重启、未提交、未推送或发布。
11. 用户在保留 debug 包后反馈“重启车机三次，测试通过”，本次网易云上电自动恢复主测记为通过；该结果来自用户真实车机重启确认，与前述 AI 连续播放观察分开记录。结合已通过的本机测试、实装 smoke 和三次重启主测，本次修复目标完成。车机继续保留 `1.0.14-icar03-test / 128`，本次 ADB 连接已断开；此次确认仅更新验收记录，未重复构建、重新安装、递增版本、提交、推送或发布。
12. 随后按用户收尾指令，将唯一版本真值递增至 `1.0.15-icar03 / 129` 并通过版本检查。`assembleRelease` 成功，包含 R8、资源收缩和发布 lint；实际正式包名、版本、单一 signer、APK v2 与非调试属性校验通过，证书与上一版正式 APK 一致。生产信任配置存在，Debug fixture 标记不存在，打包歌词 HTML 与源码一致。
13. 已导出 `03歌词-v1.0.15-icar03.apk` 与同名 ZIP 到用户指定桌面正式发布目录。APK `2389847` bytes，SHA-256 `363febf0093efebf5b43bf4275e623a1505f71730a033dce7e3d9d8d3f738ddf`；ZIP `1394821` bytes，SHA-256 `5172a365a5b61f1c9f2fc1d21ce0b3a4ad78fa8e3e06d5d2ace7f673f9a89cda`。ZIP 仅含同名 UTF-8 APK 条目，解压字节与已验签原件一致；沿用上述功能验证与三次重启结果，未再次操作车机或安装 Release。
14. 收尾同步 README、架构总纲与本地开发环境的当前版本。共享 03 APP 严格检查仍因登记版本 `1.0.10 / 124` 与实际 `1.0.15 / 129`、旧 HEAD 和工作树快照不一致而失败，未改写外部登记库，不宣称共享正式发布预检通过；本轮只在本地导出产物，未推送代码或上传产物。
...
2. 真实端点为 `com.tencent.wecarflow/com.mychery.cloudmusic.service.CloudMusicService`。它没有标准 Browser intent-filter，公开状态曾报告 `active=false / PAUSED`，但位置持续增长；补充发现与“暂停状态 + 进度增长”的仲裁规则共同保证网易云在蓝牙旧暂停会话并存时仍被选中。
3. 已按授权卸载车上旧 03 歌词包并通过 ADB 重新授权，保留数据安装最新 debug 包；`com.ninepointnine.desktoplyrics.test` 版本为 `1.0.12-icar03-test / 126`。安装、设置页启动、前台服务、媒体监听、窗口租约和致命日志 smoke 均通过。
4. 现场证据显示主应用选中 `CloudMusicService`，当前歌曲的三个输入字段已填充，时长 `248373ms`、位置持续推进，自动缓存与歌词均可用，WebView `matchingLyrics=true` 且播放状态为 `playing`。用户已确认网易云播放、暂停、恢复、切歌和歌词查找页面手动测试通过；旧爱趣听 / 蓝牙歌词残留问题已闭环。
...
1. 按用户目标模式指令，页面收敛为“开始记录”和录制后“导出报告”；删除播放 / 暂停 / 切歌 / 听感标记、播放器入口、提前结束及通知操作按钮。首次缺少媒体读取授权时由开始动作进入系统授权，返回后自动开始；后台 60 秒自然停止。可转发操作步骤见专项方案第 5 节。
2. 保留原始媒体回调、会话生命周期、原始与归一字段、时长 / 位置 / 更新时间和可访问的主应用轨迹；新增限时每 2000ms 状态与音频补读，仅记录变化并每 10 秒写检查点。补齐当前用户可见的应用身份 / 版本 / 启动组件、公开媒体服务版本 / 权限、音量 / 静音 / 输出类型 / 匿名音频配置。查询异常保存具体类型且安装状态为未知，不再统记为未安装。
3. 新报告 schema 3，读取脚本兼容 2 / 3；诊断外部轨迹最多 512 条 / 384 KiB，诊断连接上限 16，正式主链默认仍为 8。不会捕获其它应用的原始点击、音频内容或私有日志；全局音频不可归属来源、异签名正式应用内部轨迹不可读均写入报告，不把推断当作用户操作。
4. 定向 JVM 18 项、Node 2 项、diagnostic 构建与 lint、debug / androidTest 编译打包通过。独立 Android 11 模拟器的无声媒体夹具 1 项通过；没有任何手动标记，报告仍保留播放 / 暂停 / 切歌、稳定会话编号及会话移除。该次运行 60010ms、29 次补读、35 条事件，无丢弃 / 清理错误；系统文件选择器导出的 ZIP 与原件逐字节一致。夹具重建主进程导致内部租约中断，报告如实记为 interrupted。
5. Android 9 开发车机已保留数据覆盖安装新诊断包。实际页面与 Logo 检查通过，离开页面后自动录制 60041ms、29 次补读，记录 45 条事件、51 个可见启动应用和 8 个公开媒体服务；自动结束后录制服务退出、监听服务保留，ZIP 校验通过，无致命日志。真实 03T 网易云仍需用户按固定步骤回传一份报告，不把开发车机或模拟器结果冒充现场根因。
6. 唯一构建版本提升为 `1.0.12-icar03 / 126`，仅在桌面交付诊断 Debug APK 与操作文案。APK 包名 / 显示名称 / 版本 / 单 signer / v2 签名核验通过，沿用旧诊断开发签名；桌面与构建原件 SHA-256 均为 `c78491573cc476921674c8901b32096d692a5d722a1d9d6c0f0c2b1eb3758fd8`。未生成或安装新的 Release，正式用户仍为 1.0.11。
7. 项目文档、Skill、diff 检查通过。Cloud 共享检查仍因登记停留在 1.0.10 / 124 及旧 HEAD / clean 快照而失败，未改写外部登记；本轮未提交、推送、发布、清除车机数据、卸载或重启车机。

## 2026-09-07 1.0.11 打包与诊断助手更名（已导出）

1. 按用户指令将唯一版本提升至 `1.0.11-icar03 / 125`，诊断应用更名“03诊断助手”，采用官网安装教程顶栏的同源蓝色 03 Logo。图标只覆盖 diagnostic 资源，主应用包名、图标和正式签名保持原值；来源与图片摘要见网易云专项诊断方案第 6 节。
2. `assembleRelease assembleDiagnostic lintDiagnostic` 通过，Release 已完成 R8、资源收缩及发布 lint；两个 APK 实际版本、包名、单 signer 和 v2 签名通过校验。Release 生产信任配置存在、Debug fixture 标记不存在、歌词 HTML 与源码一致，正式图标与上一版 APK 相同。
3. 已在桌面现有正式包目录导出 `03歌词-v1.0.11-icar03.apk` 与同名 ZIP，在桌面导出 `03诊断助手-debug-v1.0.11-icar03.apk` 与同名 ZIP。Release APK 为 `2386967` bytes，SHA-256 `765d6eb1bf8ab5a9ed9bf8a02d17a9c501f0f74525d2214384912e130c7d26d6`；诊断 APK 为 `6684394` bytes，SHA-256 `64c0b23241739faa7ae3e4902cb199a671d03e0dc4db9c44b56273d5c65ac36a`。两个 ZIP 均为 UTF-8 中文单 APK 条目，解压字节与原件一致。
4. 新助手已在独立 Android 11 模拟器保留数据覆盖安装，标准窗口名称 / Logo / 布局截图检查通过，无致命日志；新版本报告确认 `versionCode=125`、自动到时完成且清理无错误。复用前轮媒体与诊断测试结果，未重复运行无关测试；真实 03T 仍未连接，未安装 Release 到车机。
5. 诊断 Debug 沿用开发签名，不能读取正式签名 Release 的内部观测接口，报告会明确记为签名不匹配；公开媒体与用户听感仍可采集。项目文档 / Skill / diff 检查通过；Cloud 共享登记仍为旧版本 `1.0.10 / 124`，直检存在版本与工作树快照差异，未擅自修改外部登记。用户已授权提交推送本仓库改动，未授权上传 APK 或创建线上 Release。

## 2026-09-07 03T 时长修复与网易云专项诊断（实现完成，03T 现场待确认）

1. 用户已授权目标模式实现。03T 三组公开蓝牙原始时长 `249773 / 206326 / 140000` 均按毫秒进入既有主链，删除依 Android 版本指定秒的错误假设；保留公开 Browser 发现、代理接入、统一仲裁、录音身份、时间线和歌词 `2000ms` 准入。QQ 音乐车载蓝牙歌词的动态标题不在本轮修改范围。
2. 诊断助手收敛为用户启动的 `60s` 网易云专项录制：连接前基线、公开媒体原始回调、状态发布时间、切歌与会话生命周期、用户操作 / 听感标记，以及同签名新版歌词应用的真实运行记录。删除投屏、编解码、网络发现、车辆属性与仪表显示功能及对应权限 / 依赖；不请求音频焦点、不发送播放控制。方案与用户采集流程见 `docs/plans/netease-media-diagnostics.md`。
3. 每条事件流上限 `256` 条 / `192 KiB`，淘汰与接口不可访问均显式标注；报告在私有目录原子生成 ZIP，回读校验 JSON、字节数和 SHA-256，保留最近三份。Android 11 的外部应用目录访问限制已在模拟器复现，统一改为助手私有目录与系统导出；可选 ADB 脚本通过诊断包 `run-as` 读取同一文件，不依赖 root。
4. 主链定向 JVM `140` 项、诊断归档 JVM `3` 项、Node `2` 项通过；debug / diagnostic 构建和受影响 lint 通过。独立 Android 11 模拟器已保留数据安装两个开发包，主应用基础 smoke 通过。仅在模拟器运行的公开无音频 MediaSession 夹具通过 `1` 项 instrumentation，独立诊断 ZIP 实际保留暂停 / 播放 / 切歌 / 销毁与同一会话编号，`249773 / 140000ms` 未被放大；主进程被夹具重建后，租约中断如实记录。
5. 另一轮后台录制在离开助手后持续 `60018ms` 并自动结束，用户听感标记、主应用运行快照和事件均存在，事件无截断、清理无错误、录制服务已退出。系统文件选择器导出的 ZIP 与私有原件逐字节一致，ADB 伴随脚本成功取得并验证该 ZIP。标准 `1230 x 810` 与小窗口 `640 x 400` 已截图检查；小窗口使用整页滚动，操作项可达且文本无重叠。普通 shell 访问主应用观测 Provider 被权限拒绝，符合签名边界。
6. 配置中的真实车机离线，默认安装与重连均已尝试，未把模拟器结果写成 03T 实测。剩余现场确认仅为蓝牙正常标题歌曲连续切歌自动匹配，以及车机网易云播放 / 暂停 / 恢复 / 切歌的一份完整 `60s` 报告。公开 PAUSED 不能直接证明音频真实停止，第三方内部原因仍需现场证据。
7. 唯一版本保持 `1.0.10-icar03 / 124`；本轮构建为本地开发包，未生成 staging 或 Release 分发包。实际 debug / diagnostic APK 包名、版本、同一 signer 和 APK v2 签名通过校验。项目文档 / Skill / diff 检查通过；共享 03 APP 直检只因登记 `clean` 与本轮未提交工作树 `dirty` 不一致而失败，未改写外部台账。未暂存、提交、推送、发布、清数据、卸载或重启车机。

## 2026-09-06 正式版 1.0.10 本地打包（已完成）

1. 按用户要求通过现有版本脚本将唯一 `release-version.properties` 从 `1.0.9-icar03 / 123` 递增至 `1.0.10-icar03 / 124`，同步当前版本文档；沿用正式包名 `com.ninepointnine.desktoplyrics`、原正式签名和已登记的 production 公开信任配置。
2. `assembleRelease` 成功，包含 R8 混淆、资源收缩和 `lintVitalRelease`；实际 APK 包名、版本、单 signer 与 APK v2 校验通过，证书与上一版正式 APK 一致。DEX 中生产基址、许可证公钥、keyId 和预期 signer 均匹配配置，未发现已知 Debug fixture 标记；包内歌词 HTML 与当前源码逐字节一致。
3. 已向用户指定的本地正式发布目录交付 `03歌词-v1.0.10-icar03.apk`，大小 `2383179` bytes，SHA-256 为 `012f65f7d195f79406d4ea3715451236fe4e364c077f8cf0639dc5797525630f`。
4. 同目录交付 `03歌词-v1.0.10-icar03.zip`，大小 `1390778` bytes，SHA-256 为 `92a48c311caa6dd181e64e113633fe41966c603156e6cd50a13594301bce0590`。ZIP 使用 UTF-8 中文文件名，根目录仅包含同名 APK；完整性校验通过，解压 APK 与独立 APK 字节完全一致，既有其它产物保持原状。
5. 复用前序媒体定向 `113` 项测试、debug 安装 smoke 及用户 iPhone / 爱趣听重启主测结论；本轮只升级版本并打包，未安装正式包、未操作车机、未提交、推送或上传。03T、U 盘 / 本机的剩余实机边界保持前序审计结论，不把本地打包记为新增实车验证。
6. 共享登记直检仍未通过：其版本停留在 `1.0.9-icar03 / 123`，HEAD 为旧快照；全系列 strict 检查另包含历史产物路径缺失及其它仓库快照漂移。已与实际 APK 身份、签名和字节校验分别记录，未改写外部台账。

## 2026-09-06 唯一主链最终审计与 03T 兼容核对（当前车机主测通过）

1. 用户已明确反馈爱趣听真实车机重启测试通过，前序 iPhone 蓝牙主测也已通过。当前 iCAR 03 的这两项验收已闭环；03T 按用户确认的单车验收范围保留独立实机边界。
2. 最终调用链审计确认：公开 MediaSession / Browser 统一仲裁，选中来源只进入一个 Adapter；录音代际与时间线各有唯一 tracker，标准位置、已验证 AVRCP 和检查点进入同一时间线。`LyricsPlaybackStore` 统一拥有自动 / 人工 / 恢复自动与缓存动作，`LyricsCache` 负责持久化与证明；设置和 WebView 消费同一不可变快照，播放身份与歌词按序投递。录音事实只补齐已观察的同 ID 时长，不是第二套歌词缓存或时钟。
3. 本次重新读取用户返回的 `03lyrics-03cast-diagnostic-20260905-180103.json`。可解析前缀确认 SDK 30、在线公开代理 `com.tencent.wecarflow/.player.MediaPlaybackService`、HDD / USB 及 Android 11 蓝牙 Browser；代理连接成功，原始标题动态变化而展示歌名 / 歌手稳定，时长 `205403ms`、实际位置 `109385 -> 111796ms`。内置网易云与爱趣听共用该代理接入路径，没有新增播放器白名单。原文件恰为 `131072` bytes 且末尾截断，只以完整前缀字段作证，不认定完整 `30s` 采样已取得；旧采集器固定字段列表不含 `MEDIA_ID`，不能据此认定设备不提供 ID。
4. 审计发现报告有 `10` 个公开服务，发现函数却在识别偏好 / 活动来源前先截取 `8` 个，注册表随后再截一次。已按方案第 9 节在原注册表修正：保留完整发现结果，准入后按稳定来源、当前活动包、其它合格服务分配最多 `8` 个连接。新用例先复现尾部活动来源漏接，最终媒体定向 JVM `113` 项全部通过，另覆盖完整发现与释放、03T 动态标题下录音 / 缓存身份稳定和毫秒进度；`assembleDebug` 通过。前序缓存迁移、Store、呈现与用户主测证据继续有效。
5. 已再次保留数据覆盖安装 `com.ninepointnine.desktoplyrics.test / 1.0.9-icar03-test / 123`，APK SHA-256 为 `188b04918fa3984193fff699854cf2ae412cfb884796d444d661fb8de0c806ba`。基础 smoke 通过，进程、前台歌词服务、通知监听、两类表面租约与避让监听正常，无致命日志。功能日志确认 `4` 个 Browser 会话进入合并列表并选定媒体中心；一段约 `60s` 的播放位置 `127 -> 59992ms`，代际 / 修订稳定 `3 / 4`。自然进入完整歌曲后，时长 `244459ms`、自动缓存命中，实际公开位置与原生时间线同步推进，代际 / 修订稳定 `5 / 7`；证据保留在忽略的临时目录。
6. 03T 的公开发现、代理元数据、蓝牙时长画像及共享歌词 / 缓存 / 时间线实现已纳入，尚未取得本次 03T 安装实测；剩余最小确认是对应车机的重启原曲、暂停 / 拖动与媒体通道切换。U 盘 / 本机完整播放仍未取得本次用户主测结果。共享 03 APP 检查仅旧 HEAD 台账漂移失败，未改写外部台账；临时日志级别已恢复，开发转发和先前目录搜索进程已清理。未提交、推送、发布、清数据、卸载或重启车机。

## 2026-09-06 爱趣听重启恢复的主链审计（用户真实上电主测通过）

1. 用户确认 iPhone 主测通过后，爱趣听实测仍在重启后无歌词、显示无缓存，切歌恢复。继续从录音身份、缓存索引、媒体初始快照与唯一时间线审计；方案第 7 / 8 节记录根因、物理锚点及边界。
2. 已统一 tracker、检查点与缓存三字段归一；数据库 `5 -> 6` 新增 `recording_key` 索引，旧键只作行地址。读与删除共用候选和真实毫秒校验，写入在事务中收敛重复地址并重算实际字节；保留人工原始记录。Store 每次接收当前状态，幂等不再被服务的一次性 changed 标记截断。
3. 实机同曲重建进一步确认公开桥会丢失时长并发布零进度，约 `90s` 后仍未恢复，自然下一首才正常。通过公开桥字节码确认初始零时长、`onGetRoot` 的 `STATE_NONE / 0ms` 与标准歌曲 ID。已增加有界公开录音事实持久化，只按同来源、同公开 ID 和同归一三字段恢复曾实际观察的时长；初始化零位置进入未知状态，严格绑定的检查点和后续实际位置仍进入同一时间线。preferred 与普通冷选共用可恢复状态条件，初始化 / 停止会话不再反复选入。
4. 定向 JVM 共 `180` 项通过，覆盖录音事实持久化、身份反例、重建恢复、零位置语义、AVRCP、检查点、选源、Browser、Store、缓存与设置契约；主源 / 测试源 / 受影响 androidTest 编译、`assembleDebug` 通过。前序缓存改动的 `lintDebug` 已通过，未运行日常车机 instrumentation。
5. 已保留数据覆盖安装 `com.ninepointnine.desktoplyrics.test / 1.0.9-icar03-test / 123`，基础 smoke 通过。实际数据库版本为 `6`，完整性正常、两个录音索引均存在，自动记录无缺失索引，统计 `170160` bytes 与实际载荷合计一致；三条人工记录共 `7550` bytes，升级前后原始键 / 正文 / 字节 / 更新时间摘要均为 `bf565a9b420b2290430ae6f3e17c2163dc3c44d64a5656cbd58b1dbe3f58e62e`。自动记录数随真实播放增加，不能把前后差值当成纯迁移增减。
6. 已确认新的公开 ID / 时长事实与带 ID 的检查点实际写入私有 preferences。自然播放进入完整曲目后，用应用自身重建入口验证同一首歌：时长始终为 `185202ms`，`66` 行歌词在重建后从自动缓存重新加载；位置由重建前 `87840ms` 推进至 `92901 / 102007 / 110129 / 118169 / 126248ms`，覆盖约 `38s`，活动行由 `28` 推进至 `44`。重建后的代际 / 查询修订稳定为 `10 / 17`，单一前台服务、单一 Overlay 与通知 / 避让监听正常，未发现致命日志。该次重建中公开时长及位置正常持续发布；缺失快照的持久补齐由定向测试和真实持久记录验证，服务重建本身不能证明真实开机时序。用户随后已明确反馈真实车机重启主测通过。
7. 项目文档、Skill 与 `git diff --check` 通过；共享 03 APP 检查仍仅因登记的旧 HEAD 与当前提交不一致而失败，未改写共享台账。真实车机重启由用户在收到明确通知后操作并反馈通过；U 盘 / 本机完整场景和 03T 对应实机仍未取得本次主测结果。临时日志级别与开发端口转发在交接前恢复，未留下采集进程；最终审计与本轮新增验证见上方记录，未提交、推送、发布、清数据、卸载或重启车机。

## 2026-09-06 媒体与歌词唯一主链重构（iPhone 主测通过，爱趣听重启问题继续施工）

1. 已按用户最新口径调整：正在播放且有匹配歌词时，未知进度先预览第一句；可信位置到达立即定位。方案、物理锚点、阈值和验证入口见 `docs/plans/media-lyrics-single-chain.md`。
2. `MediaPlaybackAdapter` 统一录音代际和时间线入口；标准位置、AVRCP 与检查点共用 `MediaSessionTimelineTracker`，移除独立蓝牙时钟。AVRCP 绑定录音代际，拒绝未来 / 过期事件及后续旧帧回滚；检查点补齐时长与身份约束。
3. `LyricsPlaybackStore` 统一自动 / 人工解析与缓存动作，主线程快照和串行 IO 隔离切歌、人工选择、删除及晚到写入；缓存正文和设置摘要从同一快照发布。WebView 一次按序接收播放及歌词，相同正文不重复发送；移除服务内旧请求桥和无调用的媒体展示辅助代码。
4. 数据库版本升级为 `5`，自动缓存新增查询身份，迁移重放证明与缓存键绑定；保留人工记录并重算统计。本机新增 13 项 Store 用例，补充时间线、迁移和设置状态用例；生产 HTML 的 80 项行为断言通过，覆盖壁纸、顶栏和窄顶栏。完整 JVM 共发现 374 项，唯一过时呈现断言已修正并定向复测通过，2 项既有公网测试跳过；`assembleDebug`、受影响 androidTest 编译及 `lintDebug` 通过。
5. 已完成保留数据 debug 覆盖安装与基础 smoke，安装身份为 `com.ninepointnine.desktoplyrics.test / 1.0.9-icar03-test / 123`。真实缓存数据库 `4 -> 5`，完整性正常，自动记录 `10 -> 6`，三条人工记录共 `7550` bytes 前后完全一致；按键排序的人工记录摘要均为 `f810c22d0700c5f4119f0a48618c1ff718c8cfbf7c47b2be9266a82b6aea3a3d`。不在日常车机运行 instrumentation，不清数据或重启车机。
6. 用户实测 iPhone 的 Apple Music、QQ、网易云均有歌词但不推进，Android 正常。只读观察确认 iPhone 的控制器反复发布旧位置并刷新时间戳；固件字节码取证发现原订阅漏掉 `PLAY_STATUS` 与 `PLAYBACK_STATUS_CHANGED` 两种已存在的公开输出。已将动作集合、原始数值 / 状态解码收敛到 `MediaPlaybackAdapter.kt`，播放状态与时间线一次归一输出，删除服务中重复的蓝牙状态与时间线 DTO。
7. 新增 14 项原始协议到时间线的闭环测试，与时间线、元数据、来源仲裁、设置契约合计 106 项定向回归全部通过；受影响 androidTest 编译与 `assembleDebug` 通过。已再次覆盖安装，基础 smoke 通过。安卓手机真实播放约 `47s` 内，完整状态位置从 `35173ms` 推进到 `81571ms`，归一时间线与 WebView 活动行一致；录音代际 / 查询修订保持 `3 / 3`，没有重复查词，另已观察自然切歌与新歌词同步。实际广播中两类新增进度事件均可接收。
8. 用户在本次覆盖安装后明确反馈“iPhone 的测试通过了”，iPhone 蓝牙主测已记录为通过。随后爱趣听主测反馈真实重启后无歌词、显示无缓存而切歌恢复，已进入上方根因审计；U 盘 / 本机完整播放仍未取得本次主测结论，03T 按已确认的单车验收范围保留独立实机验证边界。
9. 本机文档、Skill 与 Git 文本检查通过；共享 03 APP 检查沿用前序结论，仅登记的旧 HEAD 快照与当前提交不一致，未改写共享台账。临时日志级别已恢复、开发端口转发已移除，没有留下日志采集进程。未清数据、卸载、重启、提交、推送或发布，整体目标仍在进行中。

## 2026-09-06 爱趣听 / 03T 开机恢复与 iPhone 蓝牙续播时间线修复（本机与车机基础 smoke 已完成，媒体主测待执行）

1. 已确认开机恢复根因：公开 Browser 发现曾依赖短冷启动窗口，播放器晚于车机启动服务发布 `MediaBrowserService` 时，爱趣听 / 03T 网易云代理没有被接入；系统控制器先到达时还可能清除上次稳定 Browser 来源，下一次启动失去恢复提示。现将冷发现窗口延长为 `30000ms`，窗口结束后若仍无已选 / 已发布来源继续有界探测，并保留稳定 Browser 来源直到真实公开来源确认。
2. 已确认 iPhone 蓝牙根因：iOS 续播时公开控制器可能重复发布旧 `0ms` 或旧位置帧，原蓝牙时间线把这些帧当作可信锚点；同时位置广播动作名存在历史拼写差异。现新增 `BluetoothMediaSessionTimelineTracker` 作为唯一蓝牙时间线 owner：检查点恢复后忽略控制器位置，直到 AVRCP 位置事件确认；正确动作名与历史动作名都接收，真实续播和 seek 事件重新锚定本地时间线。
3. 已补充公开 Browser 无来源继续探测、蓝牙重复旧位置不覆盖检查点、AVRCP 续播 / seek 确认和恢复后旧控制器位置隔离测试；相关定向测试、完整 `testDebugUnitTest`、`assembleDebug`、项目文档检查、Skill 检查和 `git diff --check` 均通过。
4. 已执行保留数据 debug 覆盖安装与基础车机 smoke：测试包 `com.ninepointnine.desktoplyrics.test`，版本 `1.0.9-icar03-test` / `versionCode=123`；设置页、两个表面租约、窗口避让无障碍、通知监听、歌词服务恢复和致命日志检查均通过。尚未在本轮自动构造或伪造媒体播放数据；爱趣听 / 03T 网易云开机恢复和 iPhone Apple Music / QQ 音乐续播仍需用户在真实连接场景完成最小主测。
...
4. `MediaSessionTimelineTracker` 对新录音首帧超过 `2500ms` 的旧大位置保持 `timelineReady=false`，直到后续位置证据到达；未知位置不再伪造为可信 `0ms`。服务新增包级来源、歌名、歌手、专辑、时长和保存时间的 `24h` 播放检查点，仅在实时位置未知且时长差不超过 `2000ms` 时恢复，并由同一时间线 owner 锚定后续进度；同源控制器 Token 重建时标准和蓝牙时间线都忽略一次重建首帧的旧位置，下一帧真实位置立即重新锚定；用户主动停止仍清除检查点。
5. 已补充元数据优先级、03T 制作信息稳定代际、公开 Browser 无路由发现、新录音旧位置隔离、检查点兼容性、检查点恢复锚定和同源 Token 重建首帧回归测试；完整 `testDebugUnitTest` 为 `344` 项，其中 `342` 项通过、`2` 项既有公网测试按设计跳过、失败 `0`，`assembleDebug` 通过。
6. Debug 包已按最后补强重新构建：`com.ninepointnine.desktoplyrics.test`，版本 `1.0.9-icar03-test`、`versionCode 123`，产物为 `app/build/outputs/apk/debug/app-debug.apk`，SHA-256 为 `e21a339831c6b30ce656a84bc77036765f497f668dc9c2d4a70d13d09ce04c56`。本轮按用户要求未安装车机、未运行 ADB、未截图、未执行车机 smoke，待用户回到车上后再开始自动化非截图式验证。
7. `check-project-docs.mjs`、`check-skills.mjs`、`bump-release-version.mjs --check` 和 `git diff --check` 均通过；`check-03app-repository.mjs` 仅因共享登记快照仍指向 `54d8cf6` 而当前仓库已继续前进而失败，未改写共享登记库。
