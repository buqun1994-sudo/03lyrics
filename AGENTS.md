...
6. 商业 instrumentation 不属于普通收尾，不得因修改商业权益、安全存储或设备身份而自动在用户日常车机执行。默认只运行相关 JVM 测试、编译构建、保留数据安装与最简 smoke；商业 instrumentation 仅进入可清状态的一次性模拟器或独立测试设备。确需使用用户车机时，必须取得用户当次明确授权，并同时通过独立测试设备配置和脚本显式确认门禁。
